...
export function WaxFill({ geo, hex, gel }: { geo: VesselGeometry; hex: string; gel?: boolean }) {
  ...
  return (
    ...
    >
      <rect x={x} y={top} width={w} height={h} rx={rx} fill={hex} fillOpacity={gel ? 0.7 : 0.95} />
      {/* sheen */}
      <rect x={x + 8} y={top + 6} width="14" height={h - 20} rx="7" fill="#fff" fillOpacity={gel ? 0.25 : 0.18} />
      {gel && (
        <>
          {/* a couple of suspended bubbles for the "drink" look */}
          <circle cx={x + w * 0.65} cy={top + h * 0.4} r="5" fill="#fff" fillOpacity="0.4" />
          <circle cx={x + w * 0.4} cy={top + h * 0.62} r="3.5" fill="#fff" fillOpacity="0.35" />
        </>
      )}
    </motion.g>
  );
...
export function WhipTopping({ geo, hex }: { geo: VesselGeometry; hex: string }) {
  ...
  return (
    ...
    >
      ...
      {/* highlight */}
      <ellipse cx={cx - r * 0.3} cy={cy - r * 0.3} rx={r * 0.18} ry={r * 0.1} fill="#fff" fillOpacity="0.4" />
    </motion.g>
  );
}
  ...
  return (
    <defs>
      ...
      <radialGradient id="surfaceShadow" cx="50%" cy="50%" r="50%">
        <stop offset="0%" stopColor="#3A2C2A" stopOpacity="0.16" />
